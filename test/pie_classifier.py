...
inputfile = 'pie.data'
outputfile = 'pie_dt.dot'

dict_attr2vals = {'外壳大小':'', '形状':'', '馅料量':'', 'class':''}
...
def buildtree(ds, decision_tree):
    ...
    for val in optimal_attr_vals:
        ...
        if ds_sub_entropy == 0.0:
            ...
            #decision_tree[ds_attr_optimal][val] = {str(ds_sub['class'].tolist()[0]) : ''}
            ...
            #decision_tree[ds_attr_optimal][val] = {str(result_class) : ''}
    ...
    for ix in range(0, ds.shape[0], 1):
        ...
        for attr in mydt:
            if attr == '0' or attr == '1':
                ...
                for val in mydt[attr]:
                    if x_sample[attr] == val:
                        x_sample_value_found = True
                        mydt = mydt[attr][val]
                        break
                
                #we don't find any 'val' in mydt[attr] that equals to x_sample[attr],
                #so we try to classify x_sample into the dominant class in the tree rooted at mydt[attr]
                if not x_sample_value_found: 
                    x_sample_class = get_dominant_class_from_tree(mydt[attr][val])
                    #return x_sample_class
            
        if x_sample_class == x_sample['class']:
            correction_number += 1
        
    correction_rate = correction_number/ds.shape[0]
...
##class3_training_set, class3_validation_set = split_dataset(ds, series_class_to_sample_num.values[0]+series_class_to_sample_num.values[1], series_class_to_sample_num.values[2], training_ratio)
#
